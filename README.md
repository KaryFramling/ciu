...
`ciu` is available from CRAN at [https://cran.r-project.org/web/packages/ciu/index.html](https://cran.r-project.org/web/packages/ciu/index.html) and can be installed using the standard `install.packages("ciu")` command. 
...
The file [``TestCases.R``](TestCases.R) contains functions for running and testing CIU with various functions and data sets (Iris, Boston, Heart Disease, UCI Cars, Diamonds, Titanic, Adult) and machine learning models (lda, Random Forest, GBM). What is shown in the rest of this README.md file is mainly for giving a quick introduction to CIU and the use of the `ciu`package without needing to install it first.
